  ...
  return (
    <div>
      <div className="calendario-nav">
        <button className="btn-outline" style={{ padding: '8px 16px' }} onClick={mesAnterior}>← Anterior</button>
        <h3 className="calendario-mes-titulo">{MESES[mesActual]} {anioActual}</h3>
        <button className="btn-outline" style={{ padding: '8px 16px' }} onClick={mesSiguiente}>Siguiente →</button>
      </div>
      ...
      <div className="calendario-grid">
        ...
        {celdas.map((dia, i) => {
          ...
          return (
            <div key={dia} className={clases} onClick={() => !esDomingo && setDiaSeleccionado(seleccionado ? null : dia)}>
              ...
              {tieneCitas && (
                <div className="calendario-puntos">
                  {citasPorDia[dia].slice(0, 4).map((c, idx) => (
                    <div key={idx} className="calendario-punto" style={{ backgroundColor: ESTADO_ESTILOS[c.estado]?.color || 'var(--gold)' }} />
                  ))}
                  {citasPorDia[dia].length > 4 && <span className="calendario-mas">+{citasPorDia[dia].length - 4}</span>}
                </div>
              )}
            </div>
          )
      ...
      {diaSeleccionado && (
        <div className="calendario-citas-dia">
          <h3 className="calendario-citas-titulo">
            ...
            <span className="dashboard-badge">{citasDiaSeleccionado.length} citas</span>
          </h3>
              ...
              {citasDiaSeleccionado.sort((a, b) => a.hora.localeCompare(b.hora)).map(cita => (
                <div key={cita.id} className="cita-card">
                  ...
                  <div className="cita-acciones">
                    <span className="estado-badge" style={{ backgroundColor: ESTADO_ESTILOS[cita.estado].bg, color: ESTADO_ESTILOS[cita.estado].color }}>
                      {ESTADO_ESTILOS[cita.estado].label}
                    </span>
                    <select className="estado-select" value={cita.estado} onChange={e => handleCambiarEstado(cita.id, e.target.value)}>
                      {ESTADOS.map(e => <option key={e} value={e}>{e}</option>)}
                    </select>
                    <button className="btn-eliminar" onClick={() => handleEliminarCita(cita.id)}>🗑</button>
                  </div>
                </div>
              ))}
            </div>
...
function Dashboard() {
  ...
  const cargarDatos = async () => {
    ...
    const [citasData, serviciosData, barberosData] = await Promise.all([
      getCitas(), getServicios(), getBarberos()
    ])
    setCitas(citasData)
  ...
  const handleAgregarServicio = async () => {
    ...
    setNuevoServicio({ nombre: '', duracion: '', precio: '', emoji: '✂️' })
  }
  ...
  if (cargando) return (
    <div style={{ textAlign: 'center', padding: '80px', color: '#888' }}>Cargando datos...</div>
  )
        ...
        {seccion === 'citas' && (
          <div>
            ...
            <div className="filtros-container">
              <input type="date" className="filtro-fecha" value={filtroFecha} onChange={e => setFiltroFecha(e.target.value)} />
              ...
              {(filtroFecha || filtroEstado !== 'todos') && (
                <button className="filtro-limpiar" onClick={() => { setFiltroFecha(''); setFiltroEstado('todos') }}>✕ Limpiar</button>
              )}
            </div>
        ...
        {seccion === 'barberos' && (
          <div>
            ...
            <div className="filtro-barberos">
              <button className={`filtro-btn ${filtroBarbero === 'todos' ? 'activo' : ''}`} onClick={() => setFiltroBarbero('todos')}>Todos</button>
              {barberos.map(b => (
                <button key={b.id} className={`filtro-btn ${filtroBarbero === String(b.id) ? 'activo' : ''}`} onClick={() => setFiltroBarbero(String(b.id))}>
                  {b.nombre}
                </button>
              ))}
            </div>
            ...
            <div className="servicio-form-card">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 2fr auto', gap: '12px', alignItems: 'flex-end' }}>
                <div>
                  ...
                  <input className="input-field" placeholder="Nombre del barbero" value={nuevoBarbero.nombre} onChange={e => setNuevoBarbero({ ...nuevoBarbero, nombre: e.target.value })} />
                </div>
                <div>
                  <label className="login-input-label">Especialidad</label>
                  <input className="input-field" placeholder="Ej: Cortes clásicos" value={nuevoBarbero.especialidad} onChange={e => setNuevoBarbero({ ...nuevoBarbero, especialidad: e.target.value })} />
                </div>
                <div>
                  <label className="login-input-label">URL Foto (opcional)</label>
                  <input className="input-field" placeholder="https://..." value={nuevoBarbero.foto} onChange={e => setNuevoBarbero({ ...nuevoBarbero, foto: e.target.value })} />
                </div>
                <button className="btn-dorado" onClick={handleAgregarBarbero}>+ Agregar</button>
              </div>
            </div>
            <div className="citas-lista">
              {barberos.map(b => (
                <div key={b.id} className="cita-card">
                  {editandoBarbero?.id === b.id ? (
                    <div className="servicio-edit-form">
                      <input className="input-field" value={editandoBarbero.nombre} onChange={e => setEditandoBarbero({ ...editandoBarbero, nombre: e.target.value })} placeholder="Nombre" />
                      <input className="input-field" value={editandoBarbero.especialidad} onChange={e => setEditandoBarbero({ ...editandoBarbero, especialidad: e.target.value })} placeholder="Especialidad" />
                      <input className="input-field" value={editandoBarbero.foto} onChange={e => setEditandoBarbero({ ...editandoBarbero, foto: e.target.value })} placeholder="URL foto" />
                    ...
                    <>
                      <img src={b.foto || `https://api.dicebear.com/7.x/adventurer/svg?seed=${b.nombre}`} alt={b.nombre} className="barbero-foto" style={{ width: '56px', height: '56px' }} />
                      <div className="cita-info">
                        <p className="cita-cliente">{b.nombre}</p>
                        <p className="cita-detalle">✂ {b.especialidad}</p>
                      </div>
                      <div className="cita-acciones">
                        <button className="btn-outline" style={{ padding: '8px 16px', fontSize: '13px' }} onClick={() => setEditandoBarbero(b)}>✏ Editar</button>
                        <button className="btn-eliminar" onClick={() => handleEliminarBarbero(b.id)}>🗑 Eliminar</button>
                      </div>
                    </>
                  )}
                </div>
            ...
            <div className="servicio-form-card">
              ...
              <div className="servicio-form-grid">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label className="login-input-label">Nombre</label>
                  <input className="input-field" placeholder="Nombre del servicio" value={nuevoServicio.nombre} onChange={e => setNuevoServicio({ ...nuevoServicio, nombre: e.target.value })} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label className="login-input-label">Duración (min)</label>
                  <input className="input-field" placeholder="30" type="number" value={nuevoServicio.duracion} onChange={e => setNuevoServicio({ ...nuevoServicio, duracion: e.target.value })} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label className="login-input-label">Precio ($)</label>
                  <input className="input-field" placeholder="15000" type="number" value={nuevoServicio.precio} onChange={e => setNuevoServicio({ ...nuevoServicio, precio: e.target.value })} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label className="login-input-label">Emoji</label>
                  <input className="input-field" placeholder="✂️" value={nuevoServicio.emoji} onChange={e => setNuevoServicio({ ...nuevoServicio, emoji: e.target.value })} style={{ fontSize: '20px', textAlign: 'center' }} />
                </div>
                <button className="btn-dorado" onClick={handleAgregarServicio} style={{ alignSelf: 'flex-end' }}>+ Agregar</button>
              </div>
            </div>
            <div className="citas-lista">
              {servicios.map(s => (
                <div key={s.id} className="cita-card">
                  {editando?.id === s.id ? (
                    ...
                    <>
                      <div className="cita-info">
                        <p className="cita-cliente">{s.emoji || '✂️'} {s.nombre}</p>
